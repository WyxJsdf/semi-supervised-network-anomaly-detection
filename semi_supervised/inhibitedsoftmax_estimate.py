...
class ModelAutoEncoder(nn.Module):
    def __init__(self, num_features, device):
        ...
        self.classifier = nn.Linear(10, 2, bias=False)
    ...
    def forward(self, x):
        ...
        if self.supervised:
            ...
            with_inhibited = torch.cat((xx, inhibited_channel.double()), dim=1)

            return with_inhibited, x
        x = self.decoder(x)
        return x
...
class InhibitedSoftmax():

    def is_loss(self, model):
        return (
            lambda pred,aft_cauchy,y: self._cc(pred,y)
            -0.000001 * torch.log(aft_cauchy).sum()
        )
    def __init__(self, num_features):
        self._device = get_device()
        self._model = ModelAutoEncoder(num_features, self._device).double()
        self._criterion = nn.MSELoss()
        self._cc = nn.CrossEntropyLoss()
        self._criterion_classify = self.is_loss(self._model)
    ...
    def train_model(self, feature_labeled, feature_unlabeled, label, test_feature, test_label, epoch=10, batch_size=64):
        ...
        while epoch_id < epoch:
            ...
            y_, aft_cauchy = self._model(train_batch)
            self._optimizer.zero_grad()
            loss = self._criterion_classify(y_, aft_cauchy, train_label.long())
            loss.backward()
            ...
            try:
                ...
                step = 0
                train_loss = 0
                val_label, val_score, classify_score, _ = self.evaluate_model(test_feature, test_label)
                self._model.train()
                ...
                f1 = f1_score(test_label, val_label, average='weighted', pos_label=1)
    ...
    def evaluate_model(self, feature, label):
        ...
        for test_batch, test_label in test_loader:
            ...
            output, aft_cauchy = self._model(test_batch)
            ...
            output_confidence.append(confidence.cpu().numpy())
        ...
        return predicted_label, self.get_distance(feature, predicted_score), classify_score, confidence_score