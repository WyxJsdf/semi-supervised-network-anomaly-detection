...
def exec_lstm_classify(train_labeled_data, test_data, contam):
    lstmClassifier = LSTMClassifier(train_labeled_data[0].shape[2])
    lstmClassifier.train_model(train_labeled_data, test_data, epoch=20)
    predicted_label = lstmClassifier.evaluate_model(test_data)
    precision, recall, f1_score, accuracy = eval_data(test_data[1], predicted_label)
    print("precision = %.6lf\nrecall = %.6lf\nf1_score = %.6lf\naccuracy = %.6lf"
         %(precision, recall, f1_score, accuracy))


def exec_lstm_autoencoder(train_labeled_data, train_unlabeled_data, test_data, contam):
    print("now execute the model AutoEncoder by Pytorch!")
    lstmAutoencoder = LSTMAutoEncoder(train_unlabeled_data[0].shape[2], train_unlabeled_data[0].shape[1])
    lstmAutoencoder.train_model(train_labeled_data, train_unlabeled_data, test_data)
    predicted_label, predicted_score, _ = lstmAutoencoder.evaluate_model(test_data)
...
def main(args):
    ...
    # exec_lstm_classify((train_labeled_feature, train_label, train_labeled_seqlen), (test_feature, test_label, test_seqlen), args.contam)
    exec_lstm_autoencoder((train_labeled_feature, train_label, train_labeled_seqlen),
                          ...
                          (test_feature, test_label, test_seqlen), args.contam)
